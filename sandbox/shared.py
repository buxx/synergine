...
def do(pipe_package):
    data = pipe_package.getChunkedObjects()
    print('p', data)
    return 'foo'

if __name__ == '__main__':

    process_manager = KeepedAliveProcessManager(nb_process=1, target=do)
    m = Manager()
    ...
    for i in range(10):
        pipe_package = PipePackage(data)
        data_worked = process_manager.get_their_work(pipe_package)
