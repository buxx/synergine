...
class TravellerDisplay(PygameDisplay):
    ...
    def receive(self, synergy_object_manager, context):
        ...
        for town in synergy_object_manager.get_objects_by_type(Town):
            ...
            label = self._default_font.render(str(town).replace('<module.traveller.synergy.Town.Town object at', '').replace('>', ''), 1, (255,255,0))
        ...
        for traveller in synergy_object_manager.get_objects_by_type(Traveller):
            ...
            label = self._default_font.render(str(traveller).replace('<module.traveller.synergy.Traveller.Traveller object at', '').replace('>', ''), 1, (255,255,0))
